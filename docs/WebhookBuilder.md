...
**url** | **string** | The url that will listen to notifications (must be https). | [default to undefined]

## Example

```typescript
import { WebhookBuilder } from '@onfido/api';

const instance: WebhookBuilder = {
    enabled,
    events,
    environments,
    payload_version,
